...
\section{选项与表格}
    ...
    $\backslash$chf$\{$选项1$\}\{$选项2$\}\{$选项3$\}\{$选项4$\}$ \quad 或 \quad
    $\backslash$choice$\{$选项1$\}\{$选项2$\}\{$选项3$\}$

    以下为范例.

    \noindent(a) 以下数据正确的是\hfill(\qquad)
    \chf{是否速速}{水淀}{我胜多负少的发送到发立刻几}
    {我觉得副科}
    \noindent(b)您认为的是\hfill(\qquad)
   \chf{是}{不是}{还是}{就是}

   下面的三项排版不是合适的排版.

   \noindent (c) 请给出选择\hfill(\qquad)
   ...
   但下面排版是合适的.
    ...
    \vspace*{-15pt}多于4个选项的, 请使用choice和chf的组合, 但是需要更改选项字母, 方法如下.

    (1) $3+2$, 或者$3+3$型. 使用两个choice

    \noindent(a) 本选择题请选第5个\hfill (\qquad)
    \choice[]{We}{you}{he}
    \def\mychA{D}\def\mychB{E}\def\mychC{F}
    \choice[]{我}{你}{它}
    \def\mychA{A}\def\mychB{B}\def\mychC{C}

    \noindent(b) 本选择题请选第5个\hfill (\qquad)
    \choice{We}{you}{he}
    \def\mychA{D}\def\mychB{E}\def\mychC{F}
    \choice{我}{你}{}
    \def\mychA{A}\def\mychB{B}\def\mychC{C}

    \indent(2) $4+1$型. 使用chf, 然后换行直接书写.

    \noindent(c) 本选择题请选第5个\hfill (\quad)
    \chf{We}{you}{he}{it}\\\mbox{} \quad E.they

    (3) 5个选项, 但每个选项不长, 直接调用chfive, 如

    \noindent (d) 我们准备以下选项\hfill (\quad)
     \chfive{我们}{准备}{了5}{个选项}{网络机房}

    (4) $4+3$型, $x\ge2$. 使用chf和choice或chf和chf.

    \noindent(e) 本选择题请选第7个\hfill (\qquad)
    \chf{We}{you}{he}{大萨达联发科}
    \def\mychA{E}\def\mychB{F}\def\mychC{G}
    \choice{我}{你}{打卡机}
    \def\mychA{A}\def\mychB{B}\def\mychC{C}

    \noindent(f) 本选择题请选第7个\hfill (\qquad)
    \chf{We}{you}{he}{大萨达联发科}
    \def\mychA{E}\def\mychB{F}\def\mychC{G}
    \chf{我}{你}{打卡机}{}
    \def\mychA{A}\def\mychB{B}\def\mychC{C}
...
$\backslash$multicolumn 多列.\vspace*{-10pt}
\begin{table}[H]\zihao{-4}
\centering\renewcommand\arraystretch{1}  %%%%%% 设置表格行高
   \caption{\label{tabrecord}的罚款金}\medskip
    \begin{tabular}{|c|c|c|}
    \hline\hline
    科目&测试1&测试2\\ \hline
    语文&78&81\\ \hline
...
\centering\medskip
\caption{表格说明}\medskip
\begin{spacing}{1.2}\zihao{-4}
\begin{tabular}{
    |*{3}{@{}m{2\ccwd}<{\centering}@{}|}@{}m{8\ccwd}<{\centering}@{}|@{} m{9\ccwd}<{\centering}@{}|@{}m{4\ccwd}<{\centering}@{}|@{} m{7\ccwd}<{\centering}@{}|}\hline
    ...
    \\ \hline
    \end{tabular}
\end{spacing}%\vspace*{-30pt}
\end{table}

\begin{table}[htb]\zihao{-4}
\centering\renewcommand\arraystretch{1.5}  %%%%%% 设置表格行高
...
\end{table}
 ...
 \begin{table}[htb]
    ...
    \centering\zihao{-4} \caption{表格说明}\medskip
        \begin{spacing}{1}
...
\begin{table}[htb]
    \begin{spacing}{1.2}
        \centering\caption{\label{tabrecord}向量场对应的线性项特征根全为零}
        \medskip
    ...
    $P_n(g_1)>0$, $R(g_1)>0$
        ...
        &图(10) $O$为椭圆域型奇点; 沿$L_{Og_1}$有1个鞍点($\infty$); 沿$L_{Og_{2}}$有1个鞍点($\infty$)\\ \hline
    ...
    \begin{table}[htb]
    \centering\caption{\label{tabrecord}综合难度因素的水平划分}\medskip
...
\end{table}
    \begin{table}[htb]
    \centering \caption{\label{eqfig} 我是一个表格.}
...
\begin{tikzpicture}[scale=3]
    \foreach \i/\goods/\col in %%%%% 需要调整以下内容
        {30/东西/purple!20,26/Rice/blue!40,20/未知/red, 12/Wheat/yellow, 12/Other/red!40} % 输入格式: 百分比/物品名称/颜色
        {%以下内容不用修改
        \pgfmathsetmacro{\r}{(.8)};
        ...
        \draw({\r*cos(\mydegmid)},{\r*sin(\mydegmid)})-- node[sloped,font=\small]{\goods}({\r*cos(\mydegtex)}, {\r*sin(\mydegtex)});
...
\begin{center}
    ...
    \caption{\label{twosquare} 两个矩形.}\vspace*{-10pt}
    \end{center}
\end{figure}
...
\end{thebibliography}
\newpage
\section*{\centering致\quad谢}\bigskip
...
\newpage
\section*{\centering附\quad录}
\thispagestyle{plain}
\addcontentsline{toc}{section}{\protect附录}
此节可添加调查问卷、访谈记录等.
%\newpage%如果页眉显示不正常, 请取消该行注释
\end{document}
