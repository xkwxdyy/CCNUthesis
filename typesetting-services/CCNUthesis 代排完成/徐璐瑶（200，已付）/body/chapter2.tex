...
作为数学学习中的一个重要的不等式，均值不等式有多种称法，但其前的“均值”二字最广为人知。公式内容为 $H_{n} \leq G_{n} \leq A_{n} \leq Q_{n}$，其中 $H_{n} = \frac{n}{\sum_{i = 1}^{n} \frac{1}{x_{i}}} = \frac{n}{\frac{1}{x_{1}} + \frac{1}{x_{2}} + \cdots + \frac{1}{x_{n}}}$ ,被称为调和平均数； $G_{n} = \sqrt[n]{\prod_{i = 1}^{n} x_{i}} = \sqrt[n]{x_{1} x_{2} \cdots x_{n}}$ ,被称为几何平均数； $A_{n} = \frac{\sum_{i = 1}^{n} x_{i}}{n}=  \frac{x_{1}+ x_{2} + \cdots + x_{n}}{n}$,被称为算术平均数； $Q_{n} = \sqrt{\frac{\sum_{i = 1}^{n} x_{i}^{2}}{n}} = \sqrt{\frac{x_{1}^{2} + x_{2}^{2} + \cdots x_{n}^{2}}{n}}$ ,被称为平方平均数。均值不等式的二维形式在高中就得以呈现，该形式活跃于普通的高中课本习题及具有一定难度的数学竞赛中，大学数学中对其进行了更深入、更多维的证明与推广。
...
\begin{lemma}\label{lemma:均值不等式引理}
  若 $A \geq 0, B \geq 0$, 则 $(A+B)^{n} \geq A^{n}+n A^{n-1} B$, 当且仅当 $B=0$ 时等号 成立。
\end{lemma}
...
\begin{proof}[均值不等式的证明]
  ...
  \begin{enumerate}
    ...
    \item 当 $n=k+1$ 时, 不妨设 $x_{k+1}$ 为 $x_{1}, x_{2}, \cdots, x_{k}$ 中的最大者, 则有 $k x_{k+1} \geq x_{1}+x_{2}+\cdots+x_{k}$ 。 取 $S=x_{1}+x_{2}+\cdots+x_{k},$ 这时有 
    \[
      \left(\frac{x_{1}+x_{2}+\cdots+x_{k+1}}{k+1}\right)^{k+1}=\left(\frac{s}{k}+\right. \left.\frac{k x_{k+1}-S}{k(k+1)}\right)^{k+1},
    \] 由引理~\ref{lemma:均值不等式引理} 可得 
    \[
    ...
    \]
    在 $k x_{k+1}-S=0$ 且有 $x_{1}=x_{2}=\cdots=x_{k}$ 时, 也即 $x_{1}=x_{2}=\cdots=x_{k}=x_{k+1}$ 时等号可取得, 所以 $G_{n} \leq A_{n}$ 得证。
  \end{enumerate}
...
由于其应用能力的强大，均值不等式的推广极为广泛。数分教材第六章《微分中值定理及其应用》中，给出了其中一种推广——詹森（Jensen）不等式。
...
詹森不等式有多种应用领域。用于积分中，它展示出凸函数和其积分结果的联系；用于概率论中，它又可以表示概率密度函数的关系。詹森不等式在数学分析教材中给出了详细证明，该证明运用了数学归纳法。该不等式在多个领域有重要作用，如统计物理学和信息论。



\section{均值不等式例题分析}

均值不等式例题常出现于高中和大学的各类数学题目中，在其他学科里也有解题妙用。现聚焦于高中数学竞赛题目和高考类题目，给出相应例题。
...
\begin{analysis}
  该例题直截了当变形了均值不等式，该等价的方法为利用均值不等式解题时的最基本策略之一。
\end{analysis}
...
\begin{analysis}
  与上例题不同，该例题最初需改变原始式子，创造出崭新的式子结构，然后均值不等式便派上用场。使用该构造法需要时刻谨记均值不等式成立所要求的条件。
\end{analysis}
...
\begin{analysis}
  该题目为数分教材第二章《数列极限》习题中一道的改编，运用均值不等式内容中的 $H_{n} \leq G_{n} \leq A_{n}$ 这一不等式，通过夹逼法得出数列极限。由此可见，本题中利用均值不等式进行放缩的程度适当，以后的题目在运用放缩的方法时也需时刻注意放和缩的“度”。
\end{analysis}