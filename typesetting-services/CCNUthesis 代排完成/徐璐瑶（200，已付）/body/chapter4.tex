...
柯西不等式诞生于“留数”中，是柯西辛苦研究的产物。但从历史上来看，同一时期该不等式的发现者还有两位：布尼亚科夫斯基和施瓦茨。据此，该不等式又称为Cauchy-Buniakowsky-Schwarz不等式。该称法也是为了纪念该两位伟大的数学学者在此不等式中的卓越贡献。柯西形式一般形式为 \[\sum_{i=1}^{n} a_{i}{ }^{2} \sum_{i=1}^{n} b_{i}^{2} \geq\left(\sum_{i=1}^{n} a_{i} b_{i}\right)^{2},\] 当且仅当 $\frac{b_{1}}{a_{1}}=\frac{b_{2}}{a_{2}}=\cdots=\frac{b_{n}}{a_{n}}$ 或 $a_{i},  b_{i},i=1,2, \cdots, n$ ,当且仅当或中至少一方全为零时等号成立。柯西不等式的二维形式最为常见，为高中数学竞赛的基础。同时，其向量形式、三角形式和概率形式等也被广泛应用\cite{李梦2019数学竞赛中柯西不等式的教学研究}。


\section{柯西不等式的证明}

为与上文达成一致，这里仍用数学归纳法进行证明。
...
谈及柯西不等式所涉例题，那势必绕不开高中数学竞赛。该竞赛对此不等式爱不释手，现给出实例。
...
现具体来看看柯西不等式在大学数学中的巧妙运用。


\begin{example}
  \parencite{华东师大数学分析} 证明施瓦茨（Schwarz）不等式。（具体形式上文已给出） 
\end{example}
