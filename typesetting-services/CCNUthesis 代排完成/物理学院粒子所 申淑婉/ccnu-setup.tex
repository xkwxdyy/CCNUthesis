...
\ccnusetup{
  ...
  info = {
    ...
    title-line-type = constant,
      ...
      %     默认：constant
      %
    title = {相对论重离子碰撞中双喷注产生的理论研究},
      % 中文-标题
      %   会自动换行，如果换行点不满意，可以用 \\ 手动换行
      %   能概括整个学位论文的中心内容，简明、扼要
      %   论文题目一般不超过25个字，必要时可加副标题（在题目下一行以“——”打头）
    title* = {Theoretical Study of Di-Jet Production in Relativistic Heavy Ion Collisions}, 
      % 英文-标题
    author = {申淑婉},
      % 中文-作者姓名
    author* = {Shuwan Shen},
      ...
      %   适用学位类型：【硕｜博】
      %
    supervisor = {张本威 \quad 教授},
      % 中文-指导老师姓名+职称
      %   适用学位类型：【本｜硕｜博】
      %   职称：讲师｜副教授｜教授｜副研究员｜研究员
      %
    supervisor*-name = {Ben-Wei Zhang},
      % 拼音-指导老师姓名
      %   适用学位类型：【硕｜博】
      %
    ...
    department = {物理科学与技术学院},
      ...
      %   适用学位类型：【本｜硕｜博】
      %
    department* = {Institute of Particle Physics, College of Physical Science and Techmology},
      % 英文-学院名称
      %   适用学位类型：【硕｜博】
      %
    major = {理论物理},
    ...
    major* = {Theoretical Physics},
      ...
      %
    research-area = {相对论重离子碰撞},
      % 中文-研究方向
      %   适用学位类型：【博】
      %   选项参考：
      %     硕：教育大数据
      %
    research-area* = {Relativistic Heavy Ion Collisions},
      % 英文-研究方向
      %   适用学位类型：【硕｜博】
      %
    degree-type = {全日制博士},
   ...
   % degree-type* = {M.S.},
      ...
      %
    keywords = {
      量子色动力学，冷核物质效应，原子核部分子分部，相对论重离子碰撞，夸克胶子等离子体，喷注淬火
    },
      % 中文-论文关键词
      %   适用学位类型：【本｜硕｜博】
      %   关键词之间用西文逗号 “,” 隔开
      %
    keywords* = {
      Quantum Chromodynamics, Cold nuclear matter effects, Nuclear parton distributions, Relativistic heavy-ion collisions, Quark-Gluon Plasma, Jet quenching
    },
      % 英文-论文关键词
      %   适用学位类型：【本｜硕｜博】
      %   关键词之间用西文逗号 “,” 隔开
...
% 如果选择 fullwidth-stop = catcode 或 fullwidth-stop = mapping，建议将下面的6行代码取消注释，这6行代码的作用是将输入的中文逗号、分号和冒号编译为西文对应版本
...
% \AtBeginDocument{
% \ExplSyntaxOn
% \clist_put_right:Nn \l__ccnu_biblatex_options_clist {doi = false}
% \ExplSyntaxOff
% }