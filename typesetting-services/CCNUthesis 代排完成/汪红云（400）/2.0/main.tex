...
%   \hspace\labelsep
%   \normalfont\bfseries \underline{#1}
% }
\newcounter{cndesc}
\setlist[description]{
  before    = {\setcounter{cndesc}{0}},   % 每次环境开始重置计数器
  style     = nextline,                   % 标签和内容分行显示
  leftmargin= 0em,                        % 调整左边距
  itemindent=2em,
  font     = \normalfont,        % 标签字体加粗
  format    = {                           % 标签格式：中文数字 + 用户标签
    \stepcounter{cndesc}                  % 递增计数器
    \chinese{cndesc}、
  }
}
