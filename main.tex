...
%%%%% 需要的宏包可以自行调用 %%%%% 
%%%% 已经载入的宏包有(不用重复调用) %%%%
% amsmath, mathtools, amssymb, extarrows, unicode-math, 
% (数学常用的宏包都已经加载了)
% geometry(页面设置), ntheorem(定义新定理), 
...
\usepackage{physics}    % 为示例用，可删除，但正文示例文本中调用了该宏包的命令
                        % 删除时把chapter2中的正文也删除，填写自己的内容
                        % 下面的三个命令也是同理
\usepackage{xchoices}   % 排版选择题选项用，不需要排版选择题可删除，并把Appendix相关示例代码删除即可
