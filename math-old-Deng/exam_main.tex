...
\section{选项与表格}
   ...
   下面排版是合适的.
    ...
    多余4个选项的, 请使用choice和chf的组合, 但是需要更改选项字母, 方法如下.

    (1) $3+2$, 或者$3+3$型. 使用两个choice

    (a) 本选择题请选第5个\hfill (\quad)

    \choice{We}{you}{he}
    \vspace*{-23pt}\def\mychA{D}\def\mychB{E}\def\mychC{F}
    \choice{我}{你}{它}
    \vspace*{-13pt}\def\mychA{A}\def\mychB{B}\def\mychC{C}

    (b) 本选择题请选第5个\hfill (\quad)
    ...
    (c) 本选择题请选第5个\hfill (\quad)

    \chf{We}{you}{he}{it}
    E.\, they

    \newcommand{\chfive}[5]{\par\noindent
        \begin{tabular}{*{5}{@{}p{.2\textwidth}}}
                \mychA.#1&\mychB.#2&\mychC.#3&\mychD.#4&E.#5
            \end{tabular}}
    (3) 5个选项, 但每个选项不长, 直接使用表格生成, 上方已经给出命令, 直接调用,如

        (d) 我们准备以下选项\hfill (\quad)
...
\begin{table}[H]
    ...
    \begin{table}[H]
    \begin{center} \caption{\label{tabrecord}综合难度因素的水平划分}\smallskip
...
\begin{eqnarray*}
    ...
    =\Big\vert\int_{a}^{u}\frac{h(x)}{g(x)}\dx\Big\vert}\qquad\\
...
\section{单行公式和行内公式}
    ...
    \textbf{又一个例子}. 行内数学公式$a^2+b^2=3$, $\dis\lim_{x\to0}f(x)=4.$
    行间公式
        $$a^2+b^2=3,$$
        $$\lim_{x\to0}f(x)=4.$$
    行内行间公式, 有些符号是有差异的.
