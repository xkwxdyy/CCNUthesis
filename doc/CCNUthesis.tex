...
\begin{figure}[htbp]
  \centering
  \includegraphics[width = 0.5\textwidth]{cite-parencite效果.png}
...
证明环境（\env{proof}）的最后会添加证毕符号“$\square$”。对于证明如果以公式结尾或其它某些情况时“$\square$”可能会出现在新的空白行的行尾，如果想要“$\square$”出现在“有内容的”行尾，可以在想要出现的地方使用 \tn{qedhere} 命令，比如您可以在模版中测试下面代码查看效果：
...
\begin{itemize}
  \item 在任何情况下，本模板都会\emph{显式}调用以下宏包
    ...
    \begin{itemize}
      ...
      % \item \pkg{needspace}，如果 \kvopt{chapter-breakstyle}{continuous} 则会载入此包并使得 \tn{chapter} 不新起一页开始。
  ...
  \item 开启 \kvopt{style/bib-backend}{bibtex} 后，会调用 \pkg{natbib} 宏包，并依赖 \BibTeX{} 程序。参考文献样式由\pkg{gbt7714} 宏包提供 。
\end{itemize}
...
\begin{itemize}
  ...
  \item 学习其它论文模版的处理
    ...
    \end{itemize}
  \item 处理硕士模版
  \item 单面双面打印，空白，cleardoublepage？看 thuthesis 的处理
  \item 录制新的讲解视频
  \item 标题中输入命令 texofxxxx
\end{itemize}
...
% \bibitem[Knuth(1986)]{研究生院研究生学位论文规范}
% \textsc{Knuth D E}.
...
%   {systems/knuth/dist/tex/研究生院研究生学位论文规范.tex}
...
% \bibitem[()]{pandoxie2014CCNUthesislatex}
% \textsc{Pandoxie}.
